...
import { DataPaidProductToSendTelegram } from '../types/DataPaidProductToSendTelegram.type';
...
export const useSendTelegramPay = async (dataPayToSendTelegram: DataPaidProductToSendTelegram) => {
  ...
  try {
    ...
    await fetch(
      `https://api.telegram.org/bot${api_token}/sendMessage?chat_id=${my_channel_name}&text=Покупатель ${dataPayToSendTelegram?.email_customer}, телефон: ${dataPayToSendTelegram?.phone_customer || "не указан"} оплачивает: ${dataPayToSendTelegram?.title} на сумму: ${dataPayToSendTelegram?.amount} Руб. Заказ №: ${dataPayToSendTelegram?.order_id} Указал доставку: ${dataPayToSendTelegram?.address_delivery} Банк зачисления ${dataPayToSendTelegram.pay_method}`
    )
      .then(() => {
        // handle response
