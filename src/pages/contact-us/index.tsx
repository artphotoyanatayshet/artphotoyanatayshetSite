...
const ContactUs: React.FC = () => {
  ...
  useEffect(() => {
    const loadHtmlFragment = async () => {
      try {
        const response = await fetch('/fragmentContactUs.html'); // Путь к вашему HTML-файлу
        if (response.ok) {
  ...
  return (
    <div className="mt-12 flex flex-col items-center min-h-screen rounded-lg bg-gray-300">
      <h1 className="pt-10 text-4xl font-bold text-gray-800">{textJsonRu.contact_us.title}</h1>
