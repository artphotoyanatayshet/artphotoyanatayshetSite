...
export const updateFileForCard = async (path: string, fileContent: ArrayBuffer | string, message: string) => {
...
export const  handleFileUploadForCard =  (file: File, path: string) => {
  ...
  reader.onload = async (e) => {
    ...
    if (fileContent) {
      ...
      await updateFileForCard(path, fileContent, `Update ${file.name} via API`);
    }
  };
